...
public class ApartActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        addr = intent.getStringExtra("addr"); //메인에서 가져온 주소값 저장
        listview = (ListView) findViewById(R.id.apart_list);
        list = new ArrayList<>(); //리스트 생성
        adapter = new ApartAdapter(this,R.layout.list_item,list); //어댑터에 리스트 연결
        listview.setAdapter(adapter); //리스트뷰에 어댑터 연결시켜서 리스트를 뷰로 보여줌


        DatabaseAccess databaseAccess = DatabaseAccess.getInstance(getApplicationContext());
        databaseAccess.open(); //데이터베이스 오픈
        Cursor cursor = databaseAccess.getData("SELECT * FROM seoul3 WHERE type = '"+type+"' AND addr = '"+addr+"'"); //sql쿼리를 보냄
        apart(cursor); //쿼리문을 통한 데이터들을 받아옴
        databaseAccess.close(); //데이터베이스 닫음
        ...
        search_btn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if (search.equals("") && search2.equals("")){
                    Toast.makeText(getApplicationContext(),"동 또는 아파트 이름을 입력해주세요",Toast.LENGTH_SHORT).show();
                }
                if (!search.equals("") && search2.equals("")){
                    DatabaseAccess databaseAccess = DatabaseAccess.getInstance(getApplicationContext());
                    databaseAccess.open();
